...
def crop_roi(frame_bgr: np.ndarray, x: int, y: int, w: int, h: int) -> Tuple[np.ndarray, np.ndarray]:
...
def create_processed_contact_sheet(
    ...
    roi_frame, frame_with_roi = crop_roi(frame_bgr, roi_x, roi_y, roi_w, roi_h)

    resized = cv2.resize(roi_frame, (320, 240))
    display_original = cv2.resize(frame_with_roi, (320, 240))
    ...
    top = np.hstack([label(display_original, "1. ORIGINAL + ROI"), label(gray_bgr, "2. ROI GRAYSCALE")])
    ...
    stats = {
        "brightness": round(compute_brightness(roi_frame), 2),
...
def motion_capture(source: str, seconds: int = 8, threshold: int = 25, min_area: int = 500) -> Dict[str, Any]:
